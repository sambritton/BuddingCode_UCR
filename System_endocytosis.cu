...
void System::solveSystem() {
	...
	linearSpringInfoVecs.spring_constant_rep1 = 1000;
	//linearSpringInfoVecs.spring_constant_rep2 = 7.5;
	...
	//double NON_BEND_ANGLE = 0.0;
	//double BEND_ANGLE = 0.167448079;
	std::cout<<"BEND ANGLE = "<<BEND_ANGLE;
		...
		while (initial_kT > 0){
					...
					for (int w = 0; w < coordInfoVecs.num_edges; w++){
						...
						if (R1 < Influence_Range || R2 <Influence_Range){
							edge_to_ljparticle.push_back(w);//if within range, record the index of the edge.
						}
						else{
							edge_to_ljparticle.push_back(-1);//otherwise insert an -1.
						}
					}*/
					...
					for (int edge_loop = 0; edge_loop < num_edge_loop; edge_loop++) {
						...
						/*if (edge_to_ljparticle[edge] != -1){
							...
							bendingTriangleInfoVecs.initial_angle = NON_BEND_ANGLE;
						}*/

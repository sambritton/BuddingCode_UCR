...
struct LJSpringFunctor {
	...
	__device__ CVec4 operator()(const int & id) {
        ...
        if (R < Rcutoff) {
            ...
            epsilon * (-6.0 * pow(Rmin, 6.0) * (LJLocX - xLoc)/pow(R, 8.0) + 
                            6 * pow(Rmin, 3.0)  * (LJLocX - xLoc)/pow(R, 5.0) );

            forceY = //mag*(LJLocY - yLoc);
            epsilon * (-6.0 * pow(Rmin, 6.0) * (LJLocY - yLoc)/pow(R, 8.0) + 
                            6.0 * pow(Rmin, 3.0) * (LJLocY - yLoc)/pow(R, 5.0) );

            forceZ = //mag*(LJLocZ - zLoc);
            epsilon * (-6.0 * pow(Rmin, 6.0)  * (LJLocZ - zLoc)/pow(R, 8.0) + 
                            6.0 * pow(Rmin, 3.0) * (LJLocZ - zLoc)/pow(R, 5.0) );
            ...
            energy = epsilon * (pow( Rmin/R, 6.0 ) - 2.0 * pow( Rmin / R, 3.0));
        }
